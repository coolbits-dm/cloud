...
export async function POST(request: NextRequest) {
  return withRateLimit(request, 'chat', async () => {
    try {
      ...
      // Get chat response from aCursor (legacy Jean endpoint now uses aCursor)
      ...
      try {
        ...
        await prisma.chatMessage.create({
          data: {
            ...
            model: 'jean', // Keep legacy model name for database
            metadata: {
      ...
      logChatMessage('jean', response.sessionId, 'assistant', response.message.length)

      // Return response with legacy model name for compatibility
      const legacyResponse = {
        message: response.message,
        sessionId: response.sessionId,
        model: 'jean', // Legacy model name
        usage: response.usage,
      }
