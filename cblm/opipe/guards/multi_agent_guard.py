...
class MultiAgentGuard:
    ...
    def __init__(self, max_calls: int = 8, ttl_s: int = 60, max_depth: int = 3, max_iterations: int = 10):
    ...
    def _generate_call_hash(self, prompt: str, params: Dict[str, Any], parent_id: str) -> str:
        """Generate unique hash for call deduplication"""
        content = f"{prompt}:{str(sorted(params.items()))}:{parent_id}"
        return hashlib.sha256(content.encode()).hexdigest()[:16]
    
    def allow_tool_call(self, tool_name: str, prompt: str = "", params: Dict[str, Any] = None, parent_id: str = "") -> bool:
        ...
        # Record call
        self.calls[tool_name] = self.calls.get(tool_name, (0, time.time()))[0] + 1, time.time()
    ...
    def _is_duplicate_call(self, call_hash: str) -> bool:
        ...
        # Clean old entries
        self.recent_calls = {
            h: t for h, t in self.recent_calls.items() 
    ...
    async def run_subagents_with_barrier(self, tasks: list, timeout: float = 30) -> list:
