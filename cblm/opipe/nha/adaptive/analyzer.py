...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--collect-file", required=True, help="Path to policy_collect_*.json")
    p.add_argument("--out-dir", default="reports", help="Directory for output reports")
    p.add_argument("--min-scope-count", type=int, default=5, help="Min denies for scope gap")
    p.add_argument("--min-secret-count", type=int, default=3, help="Min denies for secret gap")
    p.add_argument("--min-agent-count", type=int, default=2, help="Min agents for agent gap")
    p.add_argument("--markdown", action="store_true", help="Also write Markdown summary")
    return p.parse_args()
...
def write_md(out_dir: Path, data: Dict[str, Any]) -> Path:
    ...
    if data['high_priority_gaps']:
        ...
        for gap in data['high_priority_gaps']:
            ...
            md.append(f"- **Agent/Scope**: {gap.get('agent_id', gap.get('scope', gap.get('secret', 'N/A')))}")
            md.append(f"- **Count**: {gap.get('violation_count', gap.get('deny_count', gap.get('count', 'N/A')))}")
