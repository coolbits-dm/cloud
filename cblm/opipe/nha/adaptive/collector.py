...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Collect policy violations from JSONL audit logs")
    p.add_argument("--logs-dir", default="logs", help="Directory with policy-enforcement-*.jsonl")
    p.add_argument("--out-dir", default="reports", help="Directory for output reports")
    p.add_argument("--window", default="last_24h",
    ...
    p.add_argument("--min-count", type=int, default=1, help="Only include items with count >= min-count")
    p.add_argument("--include-warn", action="store_true", help="Include WARN alongside DENY (default: only DENY)")
    p.add_argument("--markdown", action="store_true", help="Also write a Markdown summary")
    return p.parse_args()
...
def aggregate(records: Iterable[Dict[str, Any]],
    ...
    summary = {
        ...
        "agent_action_matrix": {
            agent: [{"action": a, "count": c} for a, c in cnt.most_common() if c >= min_count]
            for agent, cnt in agent_action.items()
        },
        "agent_scope_matrix": {
            agent: [{"scope": s, "count": c} for s, c in cnt.most_common() if c >= min_count]
...
def write_md(out_dir: Path, data: Dict[str, Any], window: str) -> Path:
    ...
    md.append(f"- Violations (counted): **{data['violations_total']}** (denies: {data['denies']}, warns: {data['warns_included']})")
    md.append("\n## Top Agents\n" + bullet("top_agents"))
    md.append("\n## Top Actions\n" + bullet("top_actions"))
    md.append("\n## Top Scopes (missing)\n" + bullet("top_scopes"))
    md.append("\n## Reasons\n" + bullet("top_reasons"))
...
def main() -> None:
    ...
    if not files:
        ...
        write_json(out_dir, {
            "generated_at": _utcnow().strftime(ISO),
            "total_records": 0, "violations_total": 0, "denies": 0, "warns_included": 0
        }, args.window)
        return

    # citește și filtrează
    records = (
        r for r in iter_records(files)
        if r.get("result") in (["DENY","WARN"] if args.include_warn else ["DENY"])
        and in_window(r, start, end)
    )
    summary = aggregate(records, include_warn=args.include_warn, min_count=args.min_count)
