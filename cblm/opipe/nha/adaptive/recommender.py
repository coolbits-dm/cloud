...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Generate policy recommendations from gap analysis")
    p.add_argument("--gaps-file", required=True, help="Path to policy_gaps.json")
    p.add_argument("--registry-file", default="cblm/opipe/nha/agents.yaml", help="Path to agents.yaml")
    p.add_argument("--out-file", default="cblm/opipe/nha/policy_recommendations.yaml", help="Output file")
    p.add_argument("--min-priority", default="medium", choices=["low", "medium", "high"], help="Minimum priority to include")
    p.add_argument("--dry-run", action="store_true", help="Print recommendations without writing file")
    return p.parse_args()
...
def generate_scope_recommendations(gaps: List[Dict[str, Any]], registry: Dict[str, Any]) -> List[Dict[str, Any]]:
            ...
            if len(scope_parts) >= 2:
                ...
                for agent_id, agent in agents_by_id.items():
                    ...
                    if scope_resource in agent_name.lower():
                        should_add = True
                    elif scope_resource == "rag" and agent_category in ["mlops", "infra"]:
                        should_add = True
                    ...
                    if should_add:
                        ...
                        if not has_scope:
                            recommendations.append({
                                ...
                                "violation_count": gap.get("deny_count", gap.get("violation_count", 0))
                            })
    
    return recommendations

def generate_secret_recommendations(gaps: List[Dict[str, Any]], registry: Dict[str, Any]) -> List[Dict[str, Any]]:
...
def generate_permission_recommendations(gaps: List[Dict[str, Any]], registry: Dict[str, Any]) -> List[Dict[str, Any]]:
                    ...
                    for perm in common_permissions:
                        if perm not in current_permissions:
                            recommendations.append({
                                ...
                                "priority": "medium",
                                "gap_type": gap.get("type"),
                                "violation_count": agent_gap.get("violation_count", 0)
                            })
                            break  # Only suggest one permission per agent
    
    return recommendations

def generate_agent_recommendations(gaps: List[Dict[str, Any]], registry: Dict[str, Any]) -> List[Dict[str, Any]]:
...
def generate_all_recommendations(gaps_data: Dict[str, Any], registry_data: Dict[str, Any], min_priority: str) -> Dict[str, Any]:
    ...
    all_recommendations = []
    all_recommendations.extend(generate_scope_recommendations(filtered_gaps, registry_data))
    all_recommendations.extend(generate_secret_recommendations(filtered_gaps, registry_data))
    all_recommendations.extend(generate_permission_recommendations(filtered_gaps, registry_data))
    all_recommendations.extend(generate_agent_recommendations(filtered_gaps, registry_data))
    ...
    result = {
        ...
        "summary": {
            "scope_recommendations": len([r for r in all_recommendations if r.get("action") == "add_scope"]),
            "secret_recommendations": len([r for r in all_recommendations if r.get("action") == "add_secret"]),
            "permission_recommendations": len([r for r in all_recommendations if r.get("action") == "add_permission"]),
            "agent_recommendations": len([r for r in all_recommendations if r.get("action") in ["investigate_agent", "review_agent_policy"]])
        }
    }
...
def main() -> None:
    ...
    # Generate recommendations
    recommendations = generate_all_recommendations(gaps_data, registry_data, args.min_priority)
    ...
    print(f"- Scope recommendations: {recommendations['summary']['scope_recommendations']}")
    print(f"- Secret recommendations: {recommendations['summary']['secret_recommendations']}")
    print(f"- Permission recommendations: {recommendations['summary']['permission_recommendations']}")
    print(f"- Agent recommendations: {recommendations['summary']['agent_recommendations']}")
