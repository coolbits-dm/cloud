...
def _audit(nha_id: str, action: str, result: str, reason: str, *,
           scope: Optional[str], extra: Optional[dict]) -> None:
    rec = {
...
def enforce_request(nha_id: str, action: str, payload: dict,
                    *, scope: Optional[str] = None,
                    require_secret: Optional[str] = None,
                    extras: Optional[dict] = None) -> EnforcementResult:
    """
    ...
    if ag is None:
        ...
        if FAIL_CLOSED or DENY_BY_DEFAULT:
            ...
            return EnforcementResult(False, "DENY", "unknown_agent", POLICY_VERSION, str(uuid.uuid4()), nha_id, action, scope, extras)
        if ALLOW_WARN:
            _audit(nha_id, action, "WARN", "unknown_agent", scope=scope, extra=extras)
            return EnforcementResult(True, "WARN", "unknown_agent", POLICY_VERSION, str(uuid.uuid4()), nha_id, action, scope, extras)

    # status
    ok, reason = _status_allows(ag, action)
    if not ok:
        _audit(nha_id, action, "DENY", reason, scope=scope, extra=extras)
        return EnforcementResult(False, "DENY", reason, POLICY_VERSION, str(uuid.uuid4()), nha_id, action, scope, extras)

    # permission-level action
    if not _permits_action(ag, action):
        if ALLOW_WARN:
            _audit(nha_id, action, "WARN", "permission_not_allowed", scope=scope, extra=extras)
            return EnforcementResult(True, "WARN", "permission_not_allowed", POLICY_VERSION, str(uuid.uuid4()), nha_id, action, scope, extras)
        _audit(nha_id, action, "DENY", "permission_not_allowed", scope=scope, extra=extras)
        return EnforcementResult(False, "DENY", "permission_not_allowed", POLICY_VERSION, str(uuid.uuid4()), nha_id, action, scope, extras)

    # optional scope
    if scope and not _has_scope(ag, scope):
        if ALLOW_WARN:
            _audit(nha_id, action, "WARN", "scope_not_allowed", scope=scope, extra=extras)
            return EnforcementResult(True, "WARN", "scope_not_allowed", POLICY_VERSION, str(uuid.uuid4()), nha_id, action, scope, extras)
        _audit(nha_id, action, "DENY", "scope_not_allowed", scope=scope, extra=extras)
        return EnforcementResult(False, "DENY", "scope_not_allowed", POLICY_VERSION, str(uuid.uuid4()), nha_id, action, scope, extras)
    ...
    if require_secret and not _has_secret(ag, require_secret):
        ...
        return EnforcementResult(False, "DENY", "secret_not_allowed", POLICY_VERSION, str(uuid.uuid4()), nha_id, action, scope, extras)

    # all good
    _audit(nha_id, action, "ALLOW", "ok", scope=scope, extra=extras)
    return EnforcementResult(True, "ALLOW", "ok", POLICY_VERSION, str(uuid.uuid4()), nha_id, action, scope, extras)
...
def get_audit_stats() -> dict:
    ...
    try:
        if not AUDIT_FILE.exists():
            return {"total_records": 0, "deny_count": 0, "allow_count": 0, "warn_count": 0}
