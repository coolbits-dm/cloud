...
def create_service_account(nha_id, project_id, dry_run=False):
    ...
    cmd = [
        ...
        f"NHA Agent: {nha_id}",
        "--description",
        f"Service account for NHA agent {nha_id}",
    ]
...
def sync_nha_with_gcp(nha, project_id, dry_run=False):
    ...
    # Apply permissions
    if nha.permissions:
        applied, failed = apply_iam_permissions(
            sa_email, nha.permissions, project_id, dry_run
        )
...
def validate_iam_policies():
    ...
    try:
        ...
        with open(
            "cblm/opipe/nha/policies/iam_minimum.yaml", "r", encoding="utf-8"
        ) as f:
            ...
            if category in iam_policies:
                ...
                for permission in nha.permissions:
                    if permission not in allowed_permissions:
                        errors.append(
                            f"NHA {nha.name} has unauthorized permission {permission} for category {category}"
                        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Sync NHA registry with Google Cloud IAM"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Perform dry run without making changes"
    )
    parser.add_argument(
        "--apply", action="store_true", help="Apply changes to Google Cloud"
    )
    parser.add_argument(
        "--project", default="coolbits-ai", help="Google Cloud project ID"
    )
    parser.add_argument(
        "--validate-only", action="store_true", help="Only validate IAM policies"
    )
