...
class TestEnforcer:
        ...
        self.test_registry = Registry(
            ...
            nhas=[
                NHA(
                    ...
                    owner="test",
                    status="active",
                    channels=[Channel(kind="http", endpoint="http://test", auth="hmac")],
                    capabilities=[
                        Capability(name="testing", description="Test capability", scopes=["read:test", "write:test"])
                    ],
                ...
                NHA(
                    ...
                    owner="test",
                    status="paused",
                    channels=[Channel(kind="http", endpoint="http://paused", auth="jwt")],
                    capabilities=[
                        Capability(name="readonly", description="Read-only capability", scopes=["read:paused"])
                    ],
    ...
    def test_allow_known_agent_with_valid_scope(self):
        ...
        with patch('cblm.opipe.nha.enforcer.load_yaml') as mock_load:
            ...
            result = enforce_request("nha:test-agent", "run.invoker", {}, scope="read:test")
    ...
    def test_deny_invalid_scope(self):
        ...
        with patch('cblm.opipe.nha.enforcer.load_yaml') as mock_load:
            ...
            result = enforce_request("nha:test-agent", "run.invoker", {}, scope="invalid:scope")
            assert not result.allowed
            assert result.decision == "DENY"
    ...
    def test_paused_agent_readonly(self):
        ...
        with patch('cblm.opipe.nha.enforcer.load_yaml') as mock_load:
            ...
            result = enforce_request("nha:paused-agent", "storage.objectViewer", {}, scope="read:paused")
            assert result.allowed
            assert result.decision == "ALLOW"
            
            # Write operations should be denied
            result = enforce_request("nha:paused-agent", "storage.objectCreator", {}, scope="write:paused")
            assert not result.allowed
            assert result.decision == "DENY"
    ...
    def test_secret_requirement(self):
        ...
        with patch('cblm.opipe.nha.enforcer.load_yaml') as mock_load:
            ...
            result = enforce_request("nha:test-agent", "run.invoker", {}, require_secret="nha/test-agent/hmac")
            assert result.allowed
            
            # Agent doesn't have the required secret
            result = enforce_request("nha:test-agent", "run.invoker", {}, require_secret="nha/test-agent/missing")
            assert not result.allowed
            assert result.decision == "DENY"
