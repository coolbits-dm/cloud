...
class NetworkLatencyInjector(BaseInjector):
    ...
    def start(self) -> bool:
        ...
        try:
            self.logger.info(
                f"Injecting {self.latency_ms}ms latency to {self.target['service']}"
            )
    ...
    def verify_injection(self) -> bool:
        ...
        try:
            ...
            response = requests.get(
                f"http://localhost:{self.target['port']}/_stcore/health", timeout=5
            )
            end_time = time.time()
            ...
            if actual_latency_ms > self.latency_ms * 0.8:  # Allow some tolerance
                self.logger.info(
                    f"Latency injection verified: {actual_latency_ms:.1f}ms"
                )
                return True
            else:
                self.logger.warning(
                    f"Latency injection not effective: {actual_latency_ms:.1f}ms"
                )
                return False
    ...
    def stop(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(
                    f"Container {self.container_name} not found during cleanup"
                )
                return True
            ...
            if result.returncode != 0:
                self.logger.warning(
                    f"Failed to remove network latency: {result.stderr}"
                )
    ...
    def safety_guard(self) -> bool:
        ...
        try:
            response = requests.get(
                f"http://localhost:{self.target['port']}/_stcore/health", timeout=2
            )
            if response.status_code != 200:
                self.logger.warning(
                    "Safety guard: Service unhealthy, stopping injection"
                )
                self.stop()
                return False
            return True
            ...
            self.logger.warning(
                f"Safety guard: Service unreachable, stopping injection: {e}"
            )
            self.stop()
            return False
...
class ServiceKillInjector(BaseInjector):
    ...
    def verify_injection(self) -> bool:
        ...
        try:
            response = requests.get(
                f"http://localhost:{self.target.get('port', 8501)}/_stcore/health",
                timeout=2,
            )
            if response.status_code != 200:
                ...
                self.logger.warning(
                    "Service kill injection not effective: service still responding"
                )
                return False
    ...
    def stop(self) -> bool:
        ...
        try:
            ...
            # Start container
            result = subprocess.run(
                ["docker", "start", self.container_name], capture_output=True, text=True
            )
    ...
    def safety_guard(self) -> bool:
        ...
        if downtime > max_downtime:
            self.logger.warning(
                f"Safety guard: Service down too long ({downtime:.1f}s), forcing restart"
            )
            self.stop()
            return False
    ...
    def verify_injection(self) -> bool:
        ...
        try:
            ...
            if cpu_percent > 70:  # High CPU usage
                self.logger.info(
                    f"CPU spike injection verified: {cpu_percent:.1f}% CPU"
                )
                return True
            else:
                self.logger.warning(
                    f"CPU spike injection not effective: {cpu_percent:.1f}% CPU"
                )
                return False
        except Exception as e:
    ...
    def safety_guard(self) -> bool:
        ...
        try:
            ...
            if load_avg > self.load_threshold:
                self.logger.warning(
                    f"Safety guard: Load average too high ({load_avg:.2f}), stopping injection"
                )
                self.stop()
                return False
            return True
    ...
    def verify_injection(self) -> bool:
        ...
        try:
            ...
            if memory_percent > 70:  # High memory usage
                self.logger.info(
                    f"Memory leak injection verified: {memory_percent:.1f}% memory"
                )
                return True
            else:
                self.logger.warning(
                    f"Memory leak injection not effective: {memory_percent:.1f}% memory"
                )
                return False
        except Exception as e:
    ...
    def safety_guard(self) -> bool:
        ...
        try:
            ...
            if memory_percent > 85:  # Critical memory usage
                self.logger.warning(
                    f"Safety guard: Memory usage too high ({memory_percent:.1f}%), stopping injection"
                )
                self.stop()
                return False
            return True
...
class DatabaseFailureInjector(BaseInjector):
    ...
    def verify_injection(self) -> bool:
        ...
        try:
            ...
            if result.returncode != 0:
                self.logger.info(
                    "Database failure injection verified: database is down"
                )
                return True
            else:
                self.logger.warning(
                    "Database failure injection not effective: database still responding"
                )
                return False
        except subprocess.TimeoutExpired:
            self.logger.info(
                "Database failure injection verified: database is unreachable"
            )
            return True
        except Exception as e:
    ...
    def safety_guard(self) -> bool:
        ...
        if downtime > max_downtime:
            self.logger.warning(
                f"Safety guard: Database down too long ({downtime:.1f}s), forcing restart"
            )
            self.stop()
            return False
...
class ExternalAPIFailureInjector(BaseInjector):
    ...
    def start(self) -> bool:
        ...
        try:
            self.logger.info(
                f"Mocking external API failures with {self.error_rate} error rate"
            )
            ...
            for endpoint in self.api_endpoints:
                try:
                    ...
                    if response.status_code >= 500:
                        self.logger.info(
                            f"External API failure injection verified: {endpoint} returning 5xx"
                        )
                        return True
                except requests.exceptions.RequestException:
                    self.logger.info(
                        f"External API failure injection verified: {endpoint} unreachable"
                    )
                    return True

            self.logger.warning(
                "External API failure injection not effective: APIs still responding"
            )
            return False
