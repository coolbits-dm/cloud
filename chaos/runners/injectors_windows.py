...
class NetworkLatencyInjector(BaseInjector):
    ...
    def start(self) -> bool:
        ...
        try:
            self.logger.info(f"Simulating {self.latency_ms}ms latency injection to {self.service}")
            self.active = True
            self.start_time = time.time()
            self.simulated_latency = True
            
            # Log injection event
            self._log_event("latency_injection_start", {
    ...
    def stop(self) -> bool:
        ...
        try:
            ...
            self._log_event("latency_injection_stop", {
                "service": self.service,
...
class ServiceKillInjector(BaseInjector):
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            # Log kill event
            self._log_event("service_kill_start", {
                "service": self.service,
    ...
    def stop(self) -> bool:
        ...
        try:
            ...
            # Log kill stop event
            self._log_event("service_kill_stop", {
                "service": self.service,
                "duration_s": time.time() - self.start_time if self.start_time else 0
            })
...
class CPUSpikeInjector(BaseInjector):
    ...
    def start(self) -> bool:
        """Simulate CPU spike"""
        try:
            self.logger.info(f"Simulating CPU spike ({self.cpu_percent}%) for {self.service}")
            self.active = True
            self.start_time = time.time()
            ...
            # Log spike event
            self._log_event("cpu_spike_start", {
                "service": self.service,
    ...
    def stop(self) -> bool:
        ...
        try:
            ...
            self._log_event("cpu_spike_stop", {
                "service": self.service,
                "duration_s": time.time() - self.start_time if self.start_time else 0
            })
