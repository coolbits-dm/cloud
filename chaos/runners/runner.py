...
from validators_windows import SLOValidator, AutoHealManager, ChaosMonitor, SLOThresholds, SLOMeasurement
    ...
    def run_experiment(self, scenario: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            while time.time() - experiment_start < target_duration:
                ...
                if self.auto_heal_manager.check_auto_heal(scenario["targets"][0]["service"], current_slo, slo_thresholds):
                    self.logger.warning("Auto-heal triggered")
                    
                    # Execute rollback
                    rollback_success = self.auto_heal_manager.execute_rollback(scenario["targets"][0]["service"])
                    result["actions"].append(f"canary_rollback: {'SUCCESS' if rollback_success else 'FAILED'}")
            ...
            result["reason"] = "SLO thresholds met" if verdict else "SLO thresholds violated"
