...
class SLOValidator:
    ...
    def fetch_slo_window(self, service: str, minutes: int = 10) -> SLOMeasurement:
        ...
        try:
            ...
            self.logger.info(
                f"Fetching SLO metrics for {service} over {minutes} minutes"
            )
            ...
            self.logger.info(
                f"SLO measurement: p95={p95_ms:.1f}ms, error_rate={error_rate:.3f}, availability={availability:.3f}"
            )
    ...
    def slo_ok(self, measurement: SLOMeasurement, thresholds: SLOThresholds) -> bool:
        ...
        try:
            self.logger.info(
                f"Checking SLO: p95={measurement.p95_ms:.1f}ms <= {thresholds.p95_ms}ms"
            )
            self.logger.info(
                f"Checking SLO: error_rate={measurement.error_rate:.3f} <= {thresholds.error_rate}"
            )
            self.logger.info(
                f"Checking SLO: availability={measurement.availability:.3f} >= {thresholds.availability}"
            )
            ...
            if overall_ok:
                ...
                if not p95_ok:
                    self.logger.error(
                        f"p95 threshold violated: {measurement.p95_ms:.1f}ms > {thresholds.p95_ms}ms"
                    )
                if not error_rate_ok:
                    self.logger.error(
                        f"Error rate threshold violated: {measurement.error_rate:.3f} > {thresholds.error_rate}"
                    )
                if not availability_ok:
                    self.logger.error(
                        f"Availability threshold violated: {measurement.availability:.3f} < {thresholds.availability}"
                    )
    ...
    def check_auto_heal_trigger(
        self, measurement: SLOMeasurement, scenario: Dict[str, Any]
    ) -> bool:
        ...
        try:
            ...
            if measurement.error_rate > trigger_threshold:
                self.logger.warning(
                    f"Auto-heal trigger: error_rate {measurement.error_rate:.3f} > {trigger_threshold}"
                )
                return True
    ...
    def canary_rollback(self, scenario: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                ["docker", "restart", container_name], capture_output=True, text=True
            )
    ...
    def verify_rollback(self, service_name: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.error(
                    f"Rollback verification: service unhealthy (status {response.status_code})"
                )
                return False
...
class ChaosMonitor:
    ...
    def log_experiment_end(self, scenario: Dict[str, Any], result: Dict[str, Any]):
        ...
        try:
            ...
            self.logger.info(
                f"Chaos experiment ended: {scenario['name']} - {result.get('verdict', 'UNKNOWN')}"
            )
            self._write_audit_log(log_entry)
    ...
    def log_slo_measurement(
        self, scenario: Dict[str, Any], measurement: SLOMeasurement
    ):
        """Log SLO measurement"""
        try:
            log_entry = {
            ...
            self.logger.info(
                f"SLO measurement: p95={measurement.p95_ms:.1f}ms, error_rate={measurement.error_rate:.3f}"
            )
            self._write_audit_log(log_entry)
    ...
    def log_auto_heal(self, scenario: Dict[str, Any], action: str, success: bool):
        ...
        try:
            ...
            self.logger.info(
                f"Auto-heal action: {action} - {'SUCCESS' if success else 'FAILED'}"
            )
            self._write_audit_log(log_entry)
...
"""
    ...
    def _generate_recommendations(
        ...
        if result.get("slo_after", {}).get("p95_ms", 0) > scenario.get("slo", {}).get(
            "p95_ms", 0
        ):
            recommendations.append("- Optimize application performance")
            recommendations.append("- Review resource allocation")

        if result.get("slo_after", {}).get("error_rate", 0) > scenario.get(
            "slo", {}
        ).get("error_rate", 0):
