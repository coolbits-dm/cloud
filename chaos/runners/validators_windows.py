...
class SLOValidator:
    ...
    def fetch_slo_window(self, service: str, minutes: int = 10) -> SLOMeasurement:
        ...
        try:
            ...
            baseline_availability = random.uniform(
                0.995, 0.999
            ...
            self.logger.info(
                f"SLO measurement: p95={measurement.p95_ms:.1f}ms, error_rate={measurement.error_rate:.3f}, availability={measurement.availability:.3f}"
            )
    ...
    def slo_ok(self, measurement: SLOMeasurement, thresholds: SLOThresholds) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"SLO check: p95={measurement.p95_ms:.1f}ms <= {thresholds.p95_ms}ms ({p95_ok}), "
...
class AutoHealManager:
    ...
    def check_auto_heal(
        self, service: str, slo_measurement: SLOMeasurement, thresholds: SLOThresholds
    ) -> bool:
        """Check if auto-healing should be triggered"""
        try:
            # Trigger rollback if SLO is severely degraded
            if (
                slo_measurement.p95_ms > thresholds.p95_ms * 2
                or slo_measurement.error_rate > thresholds.error_rate * 5
            ):
                self.logger.warning(
                    f"Auto-heal triggered for {service}: "
    ...
    def end_experiment(self, scenario_name: str, verdict: str):
        ...
        try:
            if scenario_name in self.experiments:
                ...
                duration = (
                    self.experiments[scenario_name]["end_time"]
                    - self.experiments[scenario_name]["start_time"]
                ).total_seconds()

                self.logger.info(
                    f"Chaos experiment ended: {scenario_name} -> {verdict} ({duration:.1f}s)"
                )
