...
foreach ($industry in $industries) {
    ...
    try {
        ...
        $bucketCheck = gcloud storage buckets describe gs://$bucketName --project=$ProjectId 2>$null
...
"@
        ...
        Write-Host "  Creating Data Store..." -ForegroundColor Gray
        $dataStoreId = "$industryId-rag-store"
        $dataStoreName = "projects/$ProjectId/locations/$Location/dataStores/$dataStoreId"
        
        Write-Host "  Data Store creation requires Discovery Engine API (not available via gcloud CLI)" -ForegroundColor Yellow
        ...
        Write-Host "  Search App creation requires Discovery Engine API (not available via gcloud CLI)" -ForegroundColor Yellow
...
Write-Host "`nResults saved to: rag_setup_results.json" -ForegroundColor Cyan
