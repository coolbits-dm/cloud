    ...
    def generate_organization_setup_commands(self) -> List[str]:
        ...
        commands.extend(
            [
                ...
                f"    --metadata=gpu-quota=true",
                "",
                "# Create GPU-enabled instance template",
                f"gcloud compute instance-templates create coolbits-gpu-template \\",
                f"    --machine-type=n1-standard-4 \\",
                f"    --accelerator=type=nvidia-tesla-t4,count=1 \\",
                f"    --image-family=tf-latest-gpu \\",
                ...
                f"gcloud ai datasets create \\",
                f'    --display-name="CoolBits AI Training Data" \\',
                f'    --metadata-schema-uri="gs://google-cloud-aiplatform/schema/dataset/metadata/text_1.0.0.yaml"',
                "",
                "# Create Vertex AI model endpoint",
                f"gcloud ai endpoints create \\",
                f'    --display-name="CoolBits AI Model Endpoint" \\',
                f"    --region={self.region}",
                "",
                "# 6. Security Configuration",
                "# Create secrets for API keys",
                f"gcloud secrets create openai-api-key \\",
                f'    --data-file=- <<< "YOUR_OPENAI_API_KEY"',
                "",
                f"gcloud secrets create xai-api-key \\",
                f'    --data-file=- <<< "YOUR_XAI_API_KEY"',
                "",
                "# Grant access to secrets",
                f"gcloud secrets add-iam-policy-binding openai-api-key \\",
                ...
                f"gcloud services enable logging.googleapis.com",
                "",
                "# Create monitoring dashboard",
                f"gcloud monitoring dashboards create \\",
                f"    --config-from-file=coolbits-monitoring-dashboard.json",
                "",
                "# 9. Network Configuration",
                "# Create VPC for AI workloads",
                f"gcloud compute networks create coolbits-ai-vpc \\",
                f"    --subnet-mode=regional \\",
                f"    --bgp-routing-mode=global",
                "",
                f"gcloud compute networks subnets create coolbits-ai-subnet \\",
                f"    --network=coolbits-ai-vpc \\",
                f"    --range=10.0.0.0/24 \\",
                f"    --region={self.region}",
                "",
                "# 10. Final Configuration",
                "# Set default network tags",
                f"gcloud config set compute/network coolbits-ai-vpc",
                f"gcloud config set compute/subnet coolbits-ai-subnet",
                "",
