...
class OperationalChecklist:
    ...
    def __init__(self):
        self.checklist_file = (
            f"operational_checklist_{datetime.now().strftime('%Y%m%d')}.txt"
        )
        self.results = {
    ...
    def run_daily_checks(self):
        ...
        try:
            ...
            if result.returncode == 0:
                self.log_check(
                    "DAILY", "CI Pipeline", "PASS", "Code formatting correct"
                )
            else:
                self.log_check(
                    "DAILY", "CI Pipeline", "FAIL", "Formatting issues detected"
                )
        ...
        try:
            ...
            if response.status_code == 200:
                self.log_check(
                    "DAILY", "Health Endpoints", "PASS", "Health endpoint responding"
                )
            else:
                self.log_check(
                    "DAILY",
            ...
            self.log_check(
                "DAILY", "Health Endpoints", "ERROR", f"Health check failed: {e}"
            )
        ...
        if len(log_files) > 0:
            self.log_check(
                "DAILY", "Log Files", "PASS", f"{len(log_files)} log files found"
            )
        else:
    ...
    def run_weekly_checks(self):
        ...
        try:
            ...
            if result.returncode == 0:
                self.log_check(
                    "WEEKLY", "Weekly Validation", "PASS", "All weekly tests passed"
                )
            else:
                self.log_check(
                    "WEEKLY", "Weekly Validation", "FAIL", "Some weekly tests failed"
                )
        except Exception as e:
            self.log_check(
                "WEEKLY", "Weekly Validation", "ERROR", f"Weekly validation failed: {e}"
            )
        ...
        try:
            ...
            if result.returncode == 0:
                self.log_check(
                    "WEEKLY", "Canary Deployment", "PASS", "Canary tests passed"
                )
            else:
                self.log_check(
                    "WEEKLY", "Canary Deployment", "FAIL", "Canary tests failed"
                )
        except Exception as e:
            self.log_check(
                "WEEKLY", "Canary Deployment", "ERROR", f"Canary test failed: {e}"
            )
        ...
        try:
            ...
            if result.returncode == 0:
                self.log_check(
                    "WEEKLY", "Security Tests", "PASS", "Security tests passed"
                )
            else:
                self.log_check(
                    "WEEKLY", "Security Tests", "FAIL", "Security tests failed"
                )
        except Exception as e:
            self.log_check(
                "WEEKLY", "Security Tests", "ERROR", f"Security test failed: {e}"
            )
    ...
    def run_monthly_checks(self):
        ...
        try:
            ...
            if expired_keys == 0 and expiring_soon == 0:
                self.log_check(
                    "MONTHLY", "HMAC Key Expiration", "PASS", "All keys valid"
                )
                ...
                self.log_check(
                    ...
                    "HMAC Key Expiration",
                    "WARNING",
                    f"{expiring_soon} keys expiring soon",
                )
            else:
                self.log_check(
                    "MONTHLY",
                    "HMAC Key Expiration",
                    "FAIL",
                    f"{expired_keys} keys expired",
                )

        except Exception as e:
            self.log_check(
                "MONTHLY", "HMAC Key Expiration", "ERROR", f"Key check failed: {e}"
            )
        ...
        try:
            ...
            if cpu_percent < 80 and memory_percent < 80:
                ...
                self.log_check(
                    ...
                    "System Performance",
                    "WARNING",
                    f"High usage - CPU: {cpu_percent:.1f}%, Memory: {memory_percent:.1f}%",
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--monthly", action="store_true", help="Run monthly checks only"
    )
