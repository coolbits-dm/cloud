...
DEFAULT = Roadmap(
    version="2025-09-10",
    milestones=[
        Milestone("M1","Stabilitate beton", tasks=[
        ...
        Milestone("M2","Mock→Real integrare execuții", tasks=[
        ...
        Milestone("M3","Paritate cloud & CI gates", tasks=[
            Task("M3-T1","Health cu commitSha/buildTime/node","oPyGPT03","done"),
            Task("M3-T2","CI: lint/typecheck/test/e2e + docker","oPyGPT03","done"),
            Task("M3-T3","Staging canary + rollback script","oPyGPT03","done"),
        ]),
        Milestone("M4","Hardening & UX", tasks=[
            Task("M4-T1","RBAC + HMAC pentru POST sensibile","oPyGPT03","done"),
            Task("M4-T2","Uptime checks + alerte p95/5xx","oPyGPT03","done"),
            Task("M4-T3","UI: status live, port din .runtime.json","oRunner","todo"),
        ]),
        Milestone("M5","Validare Practică", tasks=[
            ...
            Task("M5-T4","Test uptime monitor cu RAG/GPU offline","oPyGPT03","todo"),
            Task("M5-T5","Integrare HMAC key real în manager","oPyGPT03","done"),
            Task("M5-T6","Dashboard conectat la API-uri reale","oPyGPT03","done"),
        ]),
    ]
)
...
def list_all() -> None:
    ...
    for m in r.milestones:
        ...
        for t in m.tasks:
            print(f"  - {t.id:6} [{t.status:6}] {t.title}  @{t.owner}  {('* '+t.notes) if t.notes else ''}")

if __name__ == "__main__":
    # CLI: python roadmap.py list | set M2-T1 done "implemented real GPU"
    if len(sys.argv) < 2:
        print("usage: python roadmap.py list | set <TASK_ID> <todo|doing|done|blocked> [note]"); sys.exit(1)
    if sys.argv[1] == "list":
        list_all(); sys.exit(0)
    if sys.argv[1] == "set":
        _,_,tid,st,*note = sys.argv
        set_status(tid, st, " ".join(note)); sys.exit(0)
