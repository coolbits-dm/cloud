...
class SLOChecker:
    ...
    def check_p95_latency(
        self, service: str, window_minutes: int = 30, threshold_ms: float = 400
    ) -> Tuple[bool, float]:
        ...
        print(
            f"📊 {service} P95 Latency: {latest_value:.3f}s (threshold: {threshold_seconds:.3f}s)"
        )

        return is_compliant, latest_value

    def check_error_rate(
        self, service: str, window_minutes: int = 30, threshold_percent: float = 1.0
    ) -> Tuple[bool, float]:
        ...
        print(
            f"📊 {service} Error Rate: {error_rate_percent:.2f}% (threshold: {threshold_percent:.2f}%)"
        )
    ...
    def check_uptime(
        ...
        uptime_percent = (
            (sum(uptime_values) / len(uptime_values) * 100) if uptime_values else 100.0
        )
    ...
    def check_all_slos(
        ...
        # Check P95 latency
        latency_ok, latency_value = self.check_p95_latency(
            service, window_minutes, p95_threshold_ms
        )

        # Check error rate
        error_ok, error_rate = self.check_error_rate(
            service, window_minutes, error_threshold_percent
        )
        ...
        if all_ok:
            ...
            if not latency_ok:
                print("   - P95 latency exceeded threshold")
            if not error_ok:
                print("   - Error rate exceeded threshold")
            if not uptime_ok:
                print("   - Uptime below threshold")
...
def main():
    ...
    parser.add_argument(
        "--window", type=int, default=30, help="Time window in minutes (default: 30)"
    )
    parser.add_argument(
        "--p95",
        type=float,
        default=400,
        help="P95 latency threshold in ms (default: 400)",
    )
    parser.add_argument(
        "--error-rate",
        type=float,
        default=1.0,
        help="Error rate threshold in % (default: 1.0)",
    )
    parser.add_argument(
        "--project", default="coolbits-og-bridge", help="GCP project ID"
    )
