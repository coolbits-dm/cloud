...
def generate_sbom():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            # Write SBOM
            sbom_file = sbom_dir / f"coolbits-ai-sbom-{datetime.now().strftime('%Y%m%d')}.json"
        ...
        if result.returncode == 0:
            ...
            if build_result.returncode == 0:
                ...
                if syft_result.returncode == 0:
                    docker_sbom_file = sbom_dir / f"coolbits-ai-docker-sbom-{datetime.now().strftime('%Y%m%d')}.json"
        ...
        if result.returncode == 0:
            ...
            if sbom_dir.exists():
                for sbom_file in sbom_dir.glob("*.json"):
                    try:
                        subprocess.run([
                            'cosign', 'sign-blob', '--key', 'cosign.key', str(sbom_file)
                        ], check=True)
