...
class SystemStatePanel:
    ...
    def get_proof_pack_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not proof_pack_path.exists():
                return {"status": "not_found", "message": "Proof Pack not found"}
            ...
            result = subprocess.run(
                [
                    "powershell",
                    "-Command",
                    f"Get-FileHash '{proof_pack_path}' -Algorithm SHA256 | Select-Object -ExpandProperty Hash",
                ],
                capture_output=True,
                text=True,
                cwd=self.base_path,
    ...
    def get_chaos_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not chaos_reports_dir.exists():
                return {"status": "no_reports", "experiments": 0, "passed": 0}
    ...
    def get_nha_registry_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not registry_path.exists():
                return {"status": "not_found", "agent_count": 0}
    ...
    def display_panel(self):
        ...
        print(
            f"• P95 Latency: {slo['p95_latency_ms']}ms (max: {slo['thresholds']['p95_latency_max']}ms)"
        )
        print(
            f"• Error Rate: {slo['error_rate']:.3f} (max: {slo['thresholds']['error_rate_max']})"
        )
        print(
            f"• Availability: {slo['availability']:.3f} (min: {slo['thresholds']['availability_min']})"
        )
        ...
        if health["recent_denies"]:
            for i, deny in enumerate(health["recent_denies"], 1):
                print(
                    f"• {i}. {deny['timestamp']} - {deny['nha_id']} - {deny['action']}"
                )
