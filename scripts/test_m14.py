    ...
    try:
        ...
        result = subprocess.run(
            [
                "python",
                "-m",
                "cblm.opipe.nha.adaptive.analyzer",
                "--collect-file",
                "reports/policy_collect_last_24h.json",
                "--out-dir",
                "reports",
                "--markdown",
            ],
    ...
    try:
        ...
        result = subprocess.run(
            [
                "python",
                "-m",
                "cblm.opipe.nha.adaptive.recommender",
                "--gaps-file",
                "reports/policy_gaps.json",
                "--registry-file",
                "cblm/opipe/nha/agents.yaml",
                "--out-file",
                "cblm/opipe/nha/policy_recommendations.yaml",
            ],
            capture_output=True,
            text=True,
        )
        ...
        if result.returncode == 0:
            ...
            if yaml_file.exists():
                ...
                if (
                    rec_data.get("policy_recommendations", {}).get(
    ...
    try:
        ...
        result = subprocess.run(
            [
                ...
                "cblm/opipe/nha/agents.yaml",
                "--backup-file",
                "cblm/opipe/nha/out/registry.json",
                "--check-signature",
                "--auto-reload",
                "--verbose",
            ],
...
def test_file_structure():
    ...
    required_dirs = ["cblm/opipe/nha/adaptive", "reports"]
