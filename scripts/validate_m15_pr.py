...
class PolicyPRValidator:
    ...
    def validate_pr(self, pr_number: Optional[int] = None) -> bool:
        ...
        print("🔍 M15 Policy PR Bot Validation")
        print("=" * 50)
    ...
    def _check_minimal_diff(self):
        ...
        has_policies_diff = any(
            "policies/" in f and f.endswith(".yaml") for f in modified_files
        )

        if not (has_agents_diff or has_policies_diff):
            self.errors.append("PR must modify agents.yaml or policies/*.yaml")
        else:
            print("  ✅ Found modifications to agents.yaml or policies/*.yaml")
        ...
        if has_agents_diff:
            ...
            if diff_size > 50:  # More than 50 lines changed
                ...
                print(f"  ✅ Diff size reasonable ({diff_size} lines)")
    ...
    def _check_rationale(self):
        ...
        if not gaps_file.exists():
            self.errors.append(
                "Missing policy_gaps.json - rationale cannot be generated"
            )
            return
        ...
        try:
            ...
            if not gaps_data.get("all_gaps"):
                ...
                print(
                    f"  ✅ Found {len(gaps_data['all_gaps'])} policy gaps for rationale"
                )
                # Check if gaps are relevant to current changes
                high_priority = gaps_data.get("high_priority_gaps", [])
                if high_priority:
                    print(
                        f"  ✅ Found {len(high_priority)} high-priority gaps for rationale"
                    )
    ...
    def _check_proof_pack_links(self):
        ...
        print("  ✅ Proof Pack reference check (simulated)")
    ...
    def _check_trace_ids(self):
        ...
        if not logs_dir.exists():
            self.warnings.append(
                "No logs directory found - trace IDs cannot be verified"
            )
            return
        ...
        try:
            ...
            if trace_ids:
                print(f"  ✅ Found {len(trace_ids)} trace IDs in recent logs")
            else:
    ...
    def _run_validate_py(self):
        ...
        try:
            ...
            if result.returncode == 0:
                print("    ✅ Schema and business rules validation passed")
            else:
    ...
    def _run_sync_gcp_dry_run(self):
        ...
        print("  🔍 Running sync_gcp.py --dry-run...")
        ...
        try:
            ...
            if result.returncode == 0:
                print("    ✅ GCP sync dry-run passed")
            else:
                # Check if it's just a warning about dry run
                if (
                    "dry run" in result.stdout.lower()
                    or "would execute" in result.stdout.lower()
                ):
                    print("    ✅ GCP sync dry-run passed (dry run mode)")
                else:
    ...
    def _run_cosign_verify(self):
        ...
        print("  🔍 Running cosign verify-blob...")

        registry_path = (
            self.repo_path / "cblm" / "opipe" / "nha" / "out" / "registry.json"
        )
        ...
        if not pub_key.exists():
            ...
            print("    ✅ Simulated cosign verification passed")
            return
        ...
        try:
            ...
            if result.returncode == 0:
                print("    ✅ Cosign verification passed")
            else:
    ...
    def _run_verify_m13_warn(self):
        ...
        print("  🔍 Running verify_M13.ps1 in warn mode...")
        ...
        try:
            ...
            if result.returncode == 0:
                print("    ✅ M13 verification in warn mode passed")
            else:
