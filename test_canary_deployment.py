...
def test_canary_deployment():
    ...
    try:
        buggy_process = subprocess.Popen(
            [sys.executable, buggy_file], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        ...
        fixed_process = subprocess.Popen(
            [sys.executable, fixed_file], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
...
def test_rollback_script():
    ...
    try:
        ...
        assert hasattr(rollback_manager, "emergency_rollback"), (
            "emergency_rollback method not found"
        )
        assert hasattr(rollback_manager, "planned_rollback"), (
            "planned_rollback method not found"
        )
...
def test_canary_deployment_script():
    ...
    try:
        ...
        config = DeploymentConfig(
            staging_url="http://localhost:8501", production_url="http://localhost:8502"
        )
        canary_deployment = CanaryDeployment(config)
        assert hasattr(canary_deployment, "run_canary_deployment"), (
            "run_canary_deployment method not found"
        )
        assert hasattr(canary_deployment, "deploy_to_staging"), (
            "deploy_to_staging method not found"
        )
