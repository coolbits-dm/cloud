...
def integrate_real_hmac_key():
    ...
    try:
        ...
        print(f"Wrong signature test: {'✅ Rejected' if not is_invalid else '❌ Accepted'}")
...
def test_hmac_key_rotation():
    ...
    try:
        ...
        print(f"Still valid after revocation: {'❌ Yes' if is_still_valid else '✅ No'}")
...
def test_hmac_security():
    ...
    try:
        ...
        old_timestamp = str(int(time.time()) - 400)  # 400 seconds ago (beyond tolerance)
        ...
        print(f"Old timestamp (400s ago): {'❌ Valid' if is_valid_old else '✅ Invalid'}")
