...
def test_rbac_wrong_role():
    ...
    try:
        ...
        admin_has_admin_access = rbac_manager.has_permission(admin_user.id, Permission.ADMIN_ACCESS)
        admin_has_user_mgmt = rbac_manager.has_permission(admin_user.id, Permission.USER_MANAGEMENT)
        ...
        viewer_has_admin_access = rbac_manager.has_permission(viewer_user.id, Permission.ADMIN_ACCESS)
        viewer_has_user_mgmt = rbac_manager.has_permission(viewer_user.id, Permission.USER_MANAGEMENT)
        viewer_has_data_view = rbac_manager.has_permission(viewer_user.id, Permission.DATA_VIEW)
        ...
        assert not viewer_has_admin_access, "Viewer should NOT have ADMIN_ACCESS permission"
        assert not viewer_has_user_mgmt, "Viewer should NOT have USER_MANAGEMENT permission"
...
def test_jwt_authentication():
    ...
    try:
        ...
        assert payload.get('user_id') == test_user.id, "JWT should contain correct user ID"
...
def test_403_response():
    ...
    try:
        ...
        from rbac_middleware import RBACMiddleware
        from rbac_manager import Role, Permission
