...
class TestDeduplication:
    ...
    def test_dedup_5s_window(self):
        ...
        assert guard.allow_tool_call("test_tool", "same_prompt", {"param": "value"}, "parent")
        
        # Identical call should be blocked
        assert not guard.allow_tool_call("test_tool", "same_prompt", {"param": "value"}, "parent")
        
        # Different prompt should work
        assert guard.allow_tool_call("test_tool", "different_prompt", {"param": "value"}, "parent")
...
class TestParallelBarrier:
    ...
    async def test_parallel_barrier_timeout(self):
        ...
        with pytest.raises(asyncio.TimeoutError):
            await guard.run_subagents_with_barrier([slow_task(), fast_task()], timeout=30)
