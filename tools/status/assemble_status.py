...
def assemble_status(tripwire_dir: Path, registry_log: Path, 
                   proofpack_status: Path, policy_shadow: Path, 
                   slo_latest: Path, output: Path, strict: bool = False) -> dict:
    ...
    policy_shadow_data = load_json_safe(policy_shadow, {"drift_pct": 100.0, "deny_rate_pct": 100.0})
    slo_data = load_json_safe(slo_latest, {"availability_pct": 0.0, "latency_p95_ms": 999999, "error_budget_remaining_pct": 0.0})
    
    # Determine overall status
    overall = "HEALTHY"
    if strict:
        if (tripwires["critical"] > 0 or 
            not registry["cosign_verified"] or 
            not proofpack.get("fresh", False) or
            policy_shadow_data.get("drift_pct", 0) > 0.5 or
            policy_shadow_data.get("deny_rate_pct", 0) > 1.0 or
            slo_data.get("availability_pct", 0) < 99.0):
            overall = "CRITICAL"
...
def main():
    ...
    parser.add_argument("--tripwires", required=True, help="Tripwire artifacts directory")
